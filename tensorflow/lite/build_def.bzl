...
def tflite_self_contained_libs_test_suite(name):
    ...
    cc_library rule can be linked with no undefined symbols.  It also adds
    a test_suite rule that contains all the generated build tests.

    Place this rule at the bottom of a package. It will exclude any rules that
    appear after it.
